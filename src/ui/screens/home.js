...
export function mount(root, ctx) {
  ...
  function paint() {
    ...
    root.innerHTML = `
      ...
      <div class="grid3">${top.map(({ c, v }) => `<div class="tile"><div class="art" data-action="card" data-pid="${c.i}">${imgTag(imgUrl(c), c.n)}</div><div class="info"><div class="nm">${esc(displayName(c))}</div><div class="pr num">${money(v)}</div><div class="set">${esc(c.s)}</div></div></div>`).join("")}</div>` : ""}
