  ...
  function paint() {
    ...
    root.innerHTML = `
      ...
      <div class="card panel"><h3>Card database</h3>
        ...
        ${m ? `<p>Checked for new cards automatically about twice a day.</p>` : ""}
        ${state.building ? `<div class="status"><div class="spinner"></div>${b.source === "api" ? `Crawling the API: ${b.done} of ${b.total} sets · ${b.products.toLocaleString()} products` : "Downloading the pre-built catalog…"}</div>` : `${m ? `<button class="btn ghost" data-action="cat-check">Check for new cards</button>` : ""}<button class="btn ${m ? "ghost" : ""}" data-action="build">${m ? "Rebuild card database" : "Download card database"}</button>`}</div>
...
notify pgrst, 'reload schema';</pre></details>`;
  ...
  const off = delegate(root, {
    ...
    "cat-check": async () => { ctx.toast("Checking for new cards…"); const r = await app.refreshCatalogIfStale({ force: true }); ctx.toast(r.updated ? `Card database updated — ${Math.max(0, r.added).toLocaleString()} new products` : r.error ? "Couldn’t check: " + r.error : "Already up to date"); paint(); },
