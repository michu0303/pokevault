  ...
  function paint() {
    ...
    body.innerHTML = `
      ...
      <div class="card pricebox" style="flex-direction:column;align-items:stretch;gap:8px"><div style="display:flex;align-items:baseline;gap:10px;flex-wrap:wrap"><div class="pv num">${loading ? "…" : money(px)}</div><div class="pl">${loading ? "Loading prices" : px == null ? "No market price" : "Market · " + esc(pv) + (det && det.low != null ? ` · Low ${money(det.low)}` : "") + (det && det.high != null ? ` · High ${money(det.high)}` : "")}</div></div>
        ${series.length > 1 ? `<div class="pl">${deltaLine(delta)}</div>${lineChart(series, { h: 110 })}${rangeChips(range, "crange")}` : `<div class="pl">${histLoading ? "Loading price history…" : series.length === 1 ? "Recorded since " + series[0].d + " — the chart appears from the second day." : "No price history for this card yet — the daily price build records every card from the day it starts."}</div>`}</div>
