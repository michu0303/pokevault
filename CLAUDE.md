...
- `src/gapfill.js` — TCGTracking lags TCGplayer (usually the chase cards of a new set). The catalog build fills missing products from **TCGCSV** (same ids; needs a User-Agent) via `fillCatalogGaps` in `src/api.js`: filled products carry `x:1` and their prices in `fp` (`withFillPrices` overlays them on the live pricing response; live wins). `catalog-fill.json` on `catalog-data` is the running record (`open` = still missing upstream with a `since` date, `caughtUp` = log); every daily build re-checks and logs "caught up: …" lines. Only sets TCGTracking returned are filled — whole sets are never invented. The app also re-checks `catalog-meta.json` every 12 h (`app.refreshCatalogIfStale`) and swaps in newer catalogs.
